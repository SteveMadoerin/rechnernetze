...
# PINNED to the tutorial's era so the EDA (closing price, moving averages,
# daily returns, correlation heatmaps) reproduces the tutorial's period.
# The live analyse.py uses end=datetime.now(); here we lock to ~2019-2020.
end = datetime(2020, 6, 12)
start = datetime(end.year - 1, end.month, end.day)   # 2019-06-12 -> 2020-06-12
...
# Get the stock quote
# Date range PINNED to the tutorial's fixed window so results are reproducible.
# The live analyse.py uses end=datetime.now(); here we lock it to the tutorial's
# original range. end='2022-01-04' (exclusive) includes through 2022-01-03, whose
# Close of 182.01 is the max that drives the MinMax scaling — this reproduces the
# tutorial's scaled values exactly, e.g. x_train[0][:3] = [0.00439887, 0.00486851,
# 0.00584391].
df = yf.download(
    'AAPL', start='2012-01-01', end='2022-01-04',
    auto_adjust=False,         # keep a separate "Adj Close" column
    multi_level_index=False,   # keep flat columns for a single ticker
)
# Show the data
